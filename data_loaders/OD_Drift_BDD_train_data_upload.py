...
def run(project_name, access_key, secret_key, host, name):
    if name == "OD_Drift_BDD_train_data_upload":
        test_session = TestSession(
            project_name=project_name, access_key=access_key, secret_key=secret_key, host=host
        )
        # get_file(host, test_session.token, "Object_Detection/assets/OD_DRIFT_Train.csv", "Object_Detection/assets/OD_DRIFT_Train.csv")
        pd_data_frame = csv_parser(
            "/Users/abs/Desktop/ComputerVision/collins_demo/Object_Detection/assets/OD_DRIFT_Train.csv"
        )
...
if __name__ == "__main__":
    run("ObjectDetection", access_key="BhOkLXzCVX9rs6Y9cGuD",
                           secret_key="SW5JN36ELrMxQzS8GCTRYuSbtEb3s2NMvWKVxPMX", host="https://prod5.ragaai.ai", name="OD_Drift_BDD_train_data_upload")