...
def run(project_name, access_key, secret_key, host, name):
    if name == "OD_LEAKAGE_Test_data_upload":
        schema = RagaSchema()
        ...
        # get_file(host, test_session.token, "Object_Detection/assets/bdd_with_model_inferences_val.csv", "Object_Detection/assets/bdd_with_model_inferences_val.csv")
        pd_data_frame = csv_parser(
            "Object_Detection/assets/bdd_with_model_inferences_val.csv"
        )
...
if __name__ == "__main__":
    run("ObjectDetection", access_key="BhOkLXzCVX9rs6Y9cGuD",
                           secret_key="SW5JN36ELrMxQzS8GCTRYuSbtEb3s2NMvWKVxPMX", host="https://prod5.ragaai.ai", name="OD_LEAKAGE_Test_data_upload")